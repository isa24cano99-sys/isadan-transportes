-- ════════════════════════════════════════════════════════════════════════════
-- FASE 2 · Evento 8 (nómina mensual) — guards para uso real.
--   postear_nomina_mensual de Fase 1 no tenía guards (probada con rollback).
--   Se agregan dos, sin tocar la lógica contable (20 líneas devengo+aportes):
--     · PRE-CORTE: rechaza p_periodo < 2026-07-01. CA-1 ya carga las provisiones
--       de prestaciones por conductor (251015/251510/252010/252510) y los gastos
--       pre-corte están en 3610 → postear nómina pre-corte duplicaría ambos.
--     · ANTI-DUPLICADO por (conductor, mes): rechaza si ya existe un CN
--       CONTABILIZADO en el mismo periodo (YYYY-MM) con una línea de gasto (5205x)
--       del tercero conductor. Nómina es mensual: un conductor, una nómina por mes.
