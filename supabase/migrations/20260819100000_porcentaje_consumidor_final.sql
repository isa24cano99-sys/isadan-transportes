-- ════════════════════════════════════════════════════════════════════════════
-- Porcentaje conductor (61450550): el DÉBITO del costo pasa a llevar CONSUMIDOR FINAL
-- como tercero (antes: el conductor), igual que ya lo hace la comisión (61450580).
-- ÚNICO cambio de FUNCIÓN vs 20260808190000: en la rama 'porcentaje', v_leg.conductor → v_cf.
-- El CR a 13301510 (anticipo a trabajadores) SIGUE con el conductor — es el anticipo del
-- conductor, subledger por persona; no se toca.
-- ADEMÁS: el porcentaje sale de exógena — se quita el concepto 2276 (Rentas de trabajo) de la
-- cuenta 61450550, para que las FUTURAS líneas nazcan sin concepto (contab_insert_linea lo
-- snapshotea desde puc_accounts). Las 16 líneas existentes se limpian en el one-off aparte.
-- Aplicar en SQL Editor.
-- ════════════════════════════════════════════════════════════════════════════
update puc_accounts set concepto_exogena = null where codigo = '61450550';
...
begin
  ...
  loop
    if v_rec.expense_type='porcentaje' then
      perform contab_insert_linea(v_entry, '61450550', v_cf, v_placa, v_rec.amount, 0);  -- ← CONSUMIDOR FINAL (antes: v_leg.conductor)
  ...
  perform contab_insert_linea(v_entry, '13301510', v_leg.conductor, v_placa, 0, v_total);  -- CR anticipo: SIGUE con el conductor
  update legalizations set status='APROBADA' where id=p_leg_id;
