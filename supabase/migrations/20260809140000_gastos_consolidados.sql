-- ============================================================================
-- Consolidación de gastos bancarios directos en UN solo asiento (patrón Dataico:
-- un comprobante con varias líneas de débito + una de crédito al banco).
--   Tabla puente gasto_consolidado_items: registra qué bank_transactions componen un
--   asiento consolidado (una tx puede tener su CB individual O estar en un grupo, nunca
--   ambos). El anti-duplicado por transacción (bt_gasto_contabilizado) cubre las dos vías.
--   Aplicar en SQL Editor.
-- ============================================================================

-- ── Tabla puente ─────────────────────────────────────────────────────────────
create table if not exists gasto_consolidado_items (
  journal_entry_id    uuid not null references journal_entries(id) on delete cascade,
  bank_transaction_id uuid not null references bank_transactions(id),
  primary key (journal_entry_id, bank_transaction_id)
);
alter table gasto_consolidado_items disable row level security;
grant all on gasto_consolidado_items to service_role;
create index if not exists gci_bt_idx on gasto_consolidado_items (bank_transaction_id);

-- ── Anti-duplicado unificado: ¿esta bt ya se contabilizó (individual o en grupo)? ──
  ...
  select exists(
           select 1 from journal_entries e
            where e.origen_tabla = 'bank_transactions' and e.origen_id = p_bt
              and e.tipo_comprobante = 'CB' and e.estado = 'CONTABILIZADO')
      or exists(
...
declare
  ...
  v_bt uuid; v_ter uuid; v_monto numeric; v_fbt date; v_puc text; v_pre boolean; v_cf uuid;
...
begin
  ...
  foreach v_bt in array p_bt_ids loop
    ...
    if bt_gasto_contabilizado(v_bt) then
      raise exception 'El movimiento % ya está contabilizado (individual o en otro grupo)', v_bt; end if;
  end loop;
...
-- ── Anti-dup del posteo INDIVIDUAL ahora también respeta la puente ───────────
...
begin
  ...
  -- anti-duplicado unificado (individual O consolidado)
  if bt_gasto_contabilizado(p_bank_transaction_id) then
    raise exception 'El movimiento % ya está contabilizado (individual o dentro de un grupo consolidado)', p_bank_transaction_id; end if;
