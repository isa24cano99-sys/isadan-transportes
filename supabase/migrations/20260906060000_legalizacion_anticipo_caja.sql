-- ════════════════════════════════════════════════════════════════════════════
-- Legalización · anticipo entregado en efectivo (CAJA) al aprobar el viaje.
--
-- Campo opcional `anticipo_caja` en legalizations. Cuando > 0, aprobar_legalizacion
-- reconoce la entrega del anticipo DENTRO del mismo CG consolidado, con dos líneas más:
--     DB 13301510 · conductor   (reconoce el anticipo entregado)
--     CR 110505    Caja general  (la plata salió de la caja)
-- Junto con las de siempre (DB gastos / CR 13301510 consolidado). Si anticipo=costos
-- (patrón real del efectivo, saldo≈0 por viaje), el 13301510 entra y sale en el mismo
-- asiento → neto DB gastos / CR caja.
--
-- Origen SOLO CAJA (110505): el anticipo por banco sigue entrando exclusivamente por su
-- transacción bancaria real, para no romper la conciliación. Campo vacío = comportamiento
-- idéntico a hoy (el anticipo ya existía vía su bank_transaction, caso Daniel).
--
-- NO hace backfill de Daberson/Jhon (pre-corte, ya en apertura CA-1). Aplicar en SQL Editor.
-- ════════════════════════════════════════════════════════════════════════════
alter table legalizations add column if not exists anticipo_caja numeric;
...
begin
  select l.id, l.status, l.date, l.driver_id, d.tercero_id as conductor, v.plate as placa, l.anticipo_caja
  ...
  -- nada postable Y sin anticipo en efectivo → aprobar sin asiento
  if v_postable <= 0 and coalesce(v_leg.anticipo_caja,0) <= 0 then
    update legalizations set status='APROBADA' where id=p_leg_id;
  ...
  loop
    if v_rec.expense_type='porcentaje' then
      perform contab_insert_linea(v_entry, '61450550', v_cf, v_placa, v_rec.amount, 0);
  ...
  -- CR consolidado del anticipo aplicado a los costos (solo si hubo costos)
  if v_total > 0 then
    perform contab_insert_linea(v_entry, '13301510', v_leg.conductor, v_placa, 0, v_total);  -- CR anticipo · conductor
  end if;

  -- Anticipo entregado en efectivo AL aprobar (opcional): reconoce la entrega desde caja
  if coalesce(v_leg.anticipo_caja,0) > 0 then
    perform contab_insert_linea(v_entry, '13301510', v_leg.conductor, v_placa, v_leg.anticipo_caja, 0);  -- DB reconoce anticipo
    perform contab_insert_linea(v_entry, '110505',   null,          null,    0, v_leg.anticipo_caja);    -- CR Caja general
    v_posted := v_posted + 1;
  end if;

  update legalizations set status='APROBADA' where id=p_leg_id;
  return jsonb_build_object('posted', v_posted, 'skipped', v_skipped, 'total', v_total,
                            'anticipo_caja', coalesce(v_leg.anticipo_caja,0), 'asiento', v_entry);
end; $$;
