...
import { postearPagoProveedorAction, type PagoResultado } from './actions'

type Movimiento = {
  id: string; fecha: string; monto: number; proveedor: string; descripcion: string
}

export default function PagoProveedoresClient({ movimientos }: { movimientos: Movimiento[] }) {
  ...
  const postear = async (ids: string[]) => {
    ...
    const res = await postearPagoProveedorAction(lista.map(m => ({ id: m.id, ref: `${m.proveedor} · ${m.fecha}` })))
    setResultados(res)
    setSel(new Set())
    setLoading(false)
    router.refresh()
    ...
    <div className="space-y-4">
      ...
      {movimientos.length === 0 ? (
        <p className="text-sm text-[#64748B] bg-white border border-[#E2E8F0] rounded-xl p-6">
          No hay pagos a proveedor pendientes de contabilizar.
        </p>
      ) : (
        <div className="bg-white border border-[#E2E8F0] rounded-xl overflow-hidden">
              ...
              <tbody>
                {movimientos.map(m => (
                  <tr key={m.id} className="border-b border-[#E2E8F0] last:border-0 hover:bg-[#F8FAFC]">
                    ...
                    <td className="px-3 py-2.5 text-[#0F172A]">{m.proveedor}</td>
          ...
          <div className="flex items-center justify-between gap-3 px-3 py-3 bg-[#F8FAFC] border-t border-[#E2E8F0]">
            ...
            <button onClick={() => postear([...sel])} disabled={!sel.size || loading}
              ...
              {loading ? 'Contabilizando…' : 'Contabilizar pago(s) seleccionado(s)'}
            </button>
          </div>
        </div>
      )}
