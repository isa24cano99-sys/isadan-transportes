...
import { postearComisionAction, type ComisionResultado } from './actions'

type Elegible = { id: string; fecha: string; placa: string; monto: number }

export default function ComisionClient({ elegibles }: { elegibles: Elegible[] }) {
  const router = useRouter()
  const [sel, setSel] = useState<Set<string>>(new Set())
  const [loading, setLoading] = useState(false)
  const [resultados, setResultados] = useState<ComisionResultado[]>([])
  ...
  const postear = async (ids: string[]) => {
    ...
    const res = await postearComisionAction(lista.map(e => ({
      id: e.id, placa: e.placa, monto: e.monto, fecha: e.fecha, ref: `${e.placa} · ${e.fecha}`,
    })))
    setResultados(res)
    setSel(new Set())
    setLoading(false)
    ...
    <div className="space-y-4">
      ...
      {elegibles.length === 0 ? (
        <p className="text-sm text-[#64748B] bg-white border border-[#E2E8F0] rounded-xl p-6">
          No hay comisiones empresa pendientes de contabilizar.
        </p>
      ) : (
              ...
              <tbody>
                {elegibles.map(e => (
                  <tr key={e.id} className="border-b border-[#E2E8F0] last:border-0 hover:bg-[#F8FAFC]">
                    ...
                    <td className="px-3 py-2.5 text-[#0F172A] whitespace-nowrap">{e.placa || '—'}</td>
